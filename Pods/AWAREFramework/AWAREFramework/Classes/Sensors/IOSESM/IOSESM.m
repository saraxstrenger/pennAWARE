...
- (void) setESMSchedulesWithData:(NSData *)data
    ...
    if(data.length != 0){
        ...
        dispatch_async( dispatch_get_main_queue() , ^{
            ...
            //if there are now followups pending, it is ok to refresh the ESM schedules
            BOOL refresh= ![self->esmManager hasEsmValidOrFollowupsScheduled];
            
//            if(refresh){
//                NSLog(@"will refresh -> from dispatch");
//
//            }else{
//                NSLog(@"will not refresh -> from dispatch");
//            }
