...
- (instancetype)init{
    ...
    if(self != nil){
        ...
        if (![userDefaults boolForKey:@"aware_inited"]) {
            ...
            [study setUIMode:AwareUIModeHideSettings];
...
- (bool) checkStorageUsageWithViewController:(UIViewController *) viewController  showDetail:(BOOL)detail{
    ...
    if (dictionary) {
        ...
        float amountFree = free;
        
        [AWAREEventLogger.shared logEvent:@{@"class":@"AWARECore",
                                            @"event":@"compliance check",
                                            @"total":@(total), @"free":@(free),
                                            @"used":@(total-free), @"percentage":@(percentage), @"amount available":@(amountFree)}];
        //if less than 100mb left
        if(amountFree < .1 && detail){ // %
            state = NO;
