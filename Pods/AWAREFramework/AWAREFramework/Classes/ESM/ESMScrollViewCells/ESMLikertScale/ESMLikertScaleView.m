...
- (instancetype)initWithFrame:(CGRect)frame esm:(EntityESM *)esm  displayEsm: (EntityESM *) display viewController:(UIViewController *)viewController{
    self = [super initWithFrame:frame esm:esm displayEsm: display viewController:viewController];
    
    if(self != nil){
        [self addLikertScaleElement:display withFrame:frame];
    }
...
- (instancetype)initWithFrame:(CGRect)frame esm:(EntityESM *)esm  displayEsm: (EntityESM *) display bigText: (BOOL)bigText viewController:(UIViewController *)viewController{
    ...
    if(self != nil){
        ...
        if(bigText){
            ...
            [[super viewWithTag:10] removeFromSuperview];
            super.instructionLabel = [[UILabel alloc] initWithFrame:CGRectMake(10,
                                                                               super.instructionsHeight,
                                                                               frame.size.width-10*2,
                                                                               60)];
            super.instructionLabel.text = display.esm_instructions;
            //  _instructionLabel.adjustsFontSizeToFitWidth = NO;
            super.instructionLabel.font = [super.titleLabel.font fontWithSize:24];
            super.instructionLabel.numberOfLines = 0;
            
            [super.instructionLabel sizeToFit];
            
            [super addSubview:super.instructionLabel];
            [self extendHeightOfBaseView:8];
            
            if (super.displayEntity  != nil) {
                   if (@available(iOS 13.0, *)) {
                       [super.instructionLabel setTextColor:UIColor.secondaryLabelColor];
                   } else {
                       [super.instructionLabel setTextColor:UIColor.darkGrayColor];
                   }
            }
...
//    [maxLabel setBackgroundColor:[UIColor blueColor]];
    ...
    self.mainView.frame = CGRectMake(self.mainView.frame.origin.x,
                                     self.mainView.frame.origin.y+15,
