...
extension AdvancedSettingsViewController:UITableViewDelegate{
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // debug mode
        case AdvancedSettingsIdentifiers.bedTime.rawValue:
            //1. Create the alert controller.
            let alert = UIAlertController(title: row.title, message: "Set an hour which is closest to your bedtime (0-24). Please make sure that you enter your response in 24-hour time!", preferredStyle: .alert)

            //2. Add the text field. You can configure it however you need.
            alert.addTextField { (textField) in
                textField.text = ESMScheduleManager.shared().bedtime.stringValue
                    ...
                    if textFields.count > 0 {
                        if let textField = textFields.first {
                            if let text = textField.text{
                                //if number entered is an integer
                                if let bedtimeInt = Int(text) {
                                    
                                    //handle invalid input
                                    if bedtimeInt>24 || bedtimeInt<0 {
                                        
                                        let alertController = UIAlertController(title: "Hour Invalid", message:
                                        "Please enter an hour between 0 and 24", preferredStyle: .alert)
                                        ...
                                        let bedtimeNumber = NSNumber(value:bedtimeInt)
                                    
                                        ESMScheduleManager.shared().bedtime(bedtimeNumber)
                                        self.refresh()
                                    }
                                }
                            }
            ...
            break
        // auto sync
        case AdvancedSettingsIdentifiers.wakeTime.rawValue:
        //1. Create the alert controller.
            let alert = UIAlertController(title: row.title, message: "Set an hour which is closest to the time you wake up (0-24). Please make sure that you enter your response in 24-hour time!", preferredStyle: .alert)

            //2. Add the text field. You can configure it however you need.
            alert.addTextField { (textField) in
                textField.text = ESMScheduleManager.shared().waketime.stringValue
                textField.keyboardType = UIKeyboardType.numberPad
            }
                ...
                if let textFields = alert.textFields {
                    if textFields.count > 0 {
                        if let textField = textFields.first {
                            if let text = textField.text{
                                //if number entered is an integer
                                if let waketimeInt = Int(text) {
                                    let waketimeNumber = NSNumber(value:waketimeInt)
                                    if waketimeInt>24 || waketimeInt<0 {
                                        ...
                                        ESMScheduleManager.shared().waketime(waketimeNumber)
                                        self.refresh()
                                    }
        ...
        case AdvancedSettingsIdentifiers.privacy.rawValue:
            UIApplication.shared.open(URL(string: "http://pennstressstudy.github.io/privacypolicy.html")!, options: [:]) { (isOpened) in
...
extension AdvancedSettingsViewController {
    ...
    func getAdvancedSettings() -> Array<TableRowContent>{
        let study = AWAREStudy.shared()
        let schedule = ESMScheduleManager.shared()
        let settings = [TableRowContent(type: .setting,
                                        title: "Bed Time",
                                        details: schedule.bedtime.stringValue,
                                        identifier: AdvancedSettingsIdentifiers.bedTime.rawValue),
                        TableRowContent(type: .setting,
                                        title: "Wake Time",
                                        details: schedule.waketime.stringValue,
                                        identifier: AdvancedSettingsIdentifiers.wakeTime.rawValue),
                        TableRowContent(type: .setting,
                                        title: "Quit Study",
                        ...
                        //NOT IN VISIBLE SETTINGS BUT MUST BE ADDED IN TO CHANGE VISIBLE SETTINGS BELOW
                        TableRowContent(type: .setting,
                                        title: "UI Mode",
                                        details: self.getUIModeAsString(),
                                        identifier: AdvancedSettingsIdentifiers.uiMode.rawValue),
                        
                        //END OF VISIBLE SETTINGS IN HIDE-SETTINGS VIEW --> this is the default view of the app. If you want to see the full settings in-app, change the range below to include UI Mode (above), and then switch the mode there.
                        //By default, all sensors are disabled from changing settings on them manually in the app. This can be edited by changing cell.isUserInteractionEnabled to true in the ViewController.swift file
                        TableRowContent(type: .setting,
                                        title: "Debug Mode",
        ...
        if AWAREStudy.shared().getUIMode() == AwareUIModeHideSettings{
            return Array(settings[0..<10])
        }
        return settings
    }
