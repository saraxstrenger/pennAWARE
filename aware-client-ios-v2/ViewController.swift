...
class ViewController: UIViewController {
    ...
    @IBAction func didPushRefreshButton(_ sender: UIBarButtonItem) {
        ...
        // followups pending -> refresh will tamper with data collection.
        if(ESMScheduleManager.shared().hasEsmValidOrFollowupsScheduled()){
            let alert = UIAlertController(title: "Surveys are active/pending - Unable to refresh study at this time.",
                                          message: nil, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: ""), style: .cancel, handler: nil))
            self.present(alert, animated:true , completion: nil)
           
        // no followups -> refresh is ok.
        }else{
