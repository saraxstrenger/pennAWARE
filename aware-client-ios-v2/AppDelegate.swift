...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        let moreTime = UNNotificationAction(identifier:"moreTime", title:"5 More Minutes", options:[])
        let answer = UNNotificationAction(identifier: "answer", title: "Answer Now", options: .foreground)
        
        let postpone = UNNotificationCategory(identifier: "postpone", actions: [moreTime, answer], intentIdentifiers: [], options: [])
        
        let standard = UNNotificationCategory(identifier: "default", actions: [], intentIdentifiers: [], options: [])
        UNUserNotificationCenter.current().setNotificationCategories([postpone, standard])
...
extension AppDelegate : UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter,
        ...
        if response.actionIdentifier=="moreTime" {
            
            let userInfo = response.notification.request.content.userInfo
            ESMScheduleManager.shared().postponeCurrentSignal(userInfo)
        }
